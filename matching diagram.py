...
#Cruise wing loading
WS = C_L_CR*0.5*rho_CR*V_CR**2
cruise_ws = beta_cr/getAlphaT(WS)*(C_D0_CR*0.5*rho_CR*V_CR**2/(beta_cr*WS)+beta_cr*WS/(math.pi*AR*e_CR*0.5*rho_CR*V_CR**2))

cruise = [cruise_ws for i in range(len(loadings_without0))]
...
for i in range(len(loadings_without0)):
    ...
    takeoff[i] = 1.15*getAlphaT(WS)*math.sqrt(WS/(L_TO*kT*rho_TO*g*math.pi*AR*e_TO))+4*h2/L_TO
...
def climbgradient(oswald, beta,engines,C_L):
    ...
    for i in range(len(loadings_without0)):
        ...
        result[i] = 2/engines*beta/getAlphaT(WS)*(CR/V+2*C_L/(math.pi*10*oswald))
    return result
...
for i in range(len(loadings_without0)):
    WS = loadings_without0[i]
    climbrate[i] = beta_cl/getAlphaT(WS)*math.sqrt((CR**2*rho_TO/(beta_cl*WS*2)*math.sqrt(C_D0_TO*pi*AR*e_TO)+4*C_D0_TO/(pi*AR*e_TO)))
...
plt.legend(["Approach Speed", "Landing Field Length", "Cruise", "Take Off Field Length", "Climb Gradient Takeoff", "Climb Gradient Takeoff 1 OEI", "Climb Gradient Cruise", "Climb Gradient Cruise 1 OEI", "Climb Gradient Landing", "Climb Gradient Landing 1 OEI", "Climb Rate"])
plt.show()
